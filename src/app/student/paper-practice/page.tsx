...
// "View in paper": resolves the question's ORIGINAL past-paper PDF server-side
// (from subject/year/session/paper/variant, which the filename encodes; the page
// is located by matching the question text) and opens it in the shared
// PaperModal viewer so a student can compare it against the extracted crop
// without leaving the practice list.
...
type PartScore = { label: string; earned: number; max: number; modelAnswer?: string; mistake?: string };
...
function AnswerRow({ part, score, hideLabel }: { part: PracticePart; score?: PartScore; hideLabel?: boolean }) {
  // Correct answer: the bank scheme when on file (O-Level), else the marker's
  // own model answer (A-Level, generated by the AI). Shown in green. What the
  // student got wrong for this part is shown in red above it.
  const correct = (part.answer || score?.modelAnswer || "").trim();
  const mistake = (score?.mistake || "").trim();
  const label = hideLabel ? "" : (part.label || "Answer");
  // Colour the marks by how the part went: full = green, none = red, else amber.
  const marksColor = score
    ? (score.max > 0 && score.earned >= score.max ? "var(--teal-deep)"
      : score.earned <= 0 ? "var(--coral-bright)" : "var(--amber-deep)")
    : "var(--ink-faint)";
  const marksNode = score
    ? <span style={{ flex: "none", fontWeight: 700, color: marksColor }}>{score.earned} / {score.max}</span>
    : (part.marks != null && part.marks > 0)
      ? <span style={{ flex: "none", fontWeight: 700, opacity: 0.55 }}>{part.marks} mark{part.marks === 1 ? "" : "s"}</span>
      : null;
  return (
    <div style={{ fontSize: 12.5, lineHeight: 1.5 }}>
      {(label || marksNode) && (
        <div className="flex items-baseline" style={{ justifyContent: "space-between", gap: 8 }}>
          {label ? <span style={{ fontWeight: 700, color: "var(--ink)" }}>{label}</span> : <span />}
          {marksNode}
        </div>
      )}
      {mistake && (
        <p style={{ margin: "4px 0 0", color: "var(--coral-bright)", whiteSpace: "pre-wrap" }}>
          <b>What went wrong:</b> {mistake}
        </p>
      )}
      {correct && (
        <p style={{ margin: "4px 0 0", color: "var(--teal-deep)", whiteSpace: "pre-wrap" }}>
...
function ModelAnswers({ parts = [], partScores, fallbackPoints, fallbackScore }: {
  ...
  const hasText = rows.some((r) => (r.part.answer || r.score?.modelAnswer || "").trim());
  const eyebrow = hasText ? "Answers by part" : "Marks by part";
  ...
  return (
    <div style={{ marginTop: 8, borderRadius: 10, border: "1px solid var(--line)", background: "var(--surface-2)", padding: "9px 11px" }}>
      <div className="row-between" style={{ gap: 8, alignItems: "center", cursor: "pointer" }}
        onClick={() => setOpen((v) => !v)}>
        <span className="eyebrow" style={{ color: "var(--ink-soft)" }}>{eyebrow}</span>
        <button type="button" onClick={(e) => { e.stopPropagation(); setOpen((v) => !v); }} className="btn btn-ghost btn-sm"
          aria-expanded={open} style={{ padding: "2px 8px", fontSize: 11.5, color: "var(--ink-soft)", whiteSpace: "nowrap" }}>
          <Icon name={open ? "chevron_down" : "chevron_right"} size={13} /> {toggleLabel}
        </button>
      </div>
      {open && (
        <div className="flex-col" style={{ display: "flex", gap: 0, marginTop: 8 }}>
          {rows.map(({ part, score }, index) => (
            <div key={index} style={index ? { borderTop: "1px solid var(--line)", marginTop: 10, paddingTop: 10 } : undefined}>
              <AnswerRow part={part} hideLabel={!part.label} score={score} />
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div className="card card-pad" style={{ padding: 14, ...(withheld ? { borderColor: "var(--coral-soft)" } : {}) }}>
      ...
      {(() => {
        ...
        const hasPartModelAnswers = (q.partScores ?? []).some((s) => (s.modelAnswer || "").trim());
        const hasMarkParts = list.some((p) => (p.marks ?? 0) > 0 && !isHeaderPart(list, p.label));
        // Only fall back to the flat key-points list when the marker gave no
        // per-part answers to show inside "Answers by part" above.
        return !hasBankAnswers && !hasPartModelAnswers && hasMarkParts && (q.expectedPoints?.length ?? 0) > 0
