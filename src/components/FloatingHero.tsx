...
/* -------------------------------------------------------------------------- */
/* Live-marking demo — one card, the sequence replays on a loop (via `cycle`). */
const MARK_ROWS = [
...
function LiveMarking() {
  ...
  return (
    <div className="relative overflow-hidden rounded-[1.6rem] border border-line bg-surface/95 p-5 shadow-card-hover backdrop-blur-xl md:p-6">
      {/* header */}
...
export default function FloatingHero({ user, profile, onSignUp, onExplore }: FloatingHeroProps) {
  ...
  return (
    <section ref={ref} className="relative w-full overflow-hidden bg-paper pt-28 pb-20 md:pt-32 md:pb-28">
      {/* ---- Animated aurora backdrop ---- */}
      <div className="absolute inset-0 ed-grid-bg opacity-50" />
      <motion.div
        aria-hidden
        className="pointer-events-none absolute -top-32 -left-10 h-[32rem] w-[32rem] rounded-full bg-crimson/12 blur-[130px]"
        animate={reduce ? {} : { x: [0, 40, 0], y: [0, 26, 0], scale: [1, 1.12, 1] }}
        transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        aria-hidden
        className="pointer-events-none absolute top-[10%] right-0 h-[28rem] w-[28rem] rounded-full bg-pink/12 blur-[120px]"
      ...
      <div className="relative z-10 mx-auto grid max-w-[1180px] items-center gap-12 px-5 lg:grid-cols-2 lg:gap-10">
        ...
        <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
          ...
          <h1 className="mt-5 font-display text-[2.7rem] font-semibold leading-[1.04] tracking-tight text-ink sm:text-6xl lg:text-[4rem]">
